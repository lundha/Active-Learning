...
class NeuralClassification(nn.Module):
    ...
    """
    def __init__(self, data, args, feature_extractor=None, metric='Acc', num_features=256, full_cov=False, cov_rank=None):
        super().__init__()
    ...
    def optimize(self, data, num_epochs=1000, batch_size=64, initial_lr=1e-2, freq_summary=100,
        ...
        '''
        idx_lb = data.index['labeled']
        handler = DataHandler(X=data.X[idx_lb], Y=data.Y[idx_lb], transform=self.args['transform'])
        dataloader = DataLoader(handler, shuffle=False, batch_size=200, num_workers=0)
        ...
        for epoch in range(num_epochs):
            ...
            if epoch % freq_summary == 0 or epoch == num_epochs - 1:
                ...
                print('#{} loss: {:.4f} (val: {:.4f}), kl: {:.4f}, {}: {:.4f} (val: {:.4f})'.format(
                    epoch, np.mean(losses), np.mean(losses), np.mean(kls),
                    self.metric, np.mean(performances), np.mean(performances)))
    ...
    def get_projections(self, data, J, projection='two', gamma=0, transform=None, **kwargs):
        ...
        return to_gpu(torch.sqrt(1 / torch.FloatTensor([J]))) * torch.cat(projections, dim=1), ent_x
    ...
    def _compute_predictive_posterior(self, y_pred, logits=True):
        ...
        """
        log_pred_samples = y_pred
        L = to_gpu(torch.FloatTensor([log_pred_samples.shape[0]]))
...
class LocalReparamDense(nn.Module):
    ...
    def forward(self, x, num_samples=1, squeeze=False):
        ...
        return sample_normal(mean, var, num_samples, squeeze)
    ...
    def compute_kl(self):
        ...
        # scale = 1.
        return gaussian_kl_diag(mean, torch.diag(cov), torch.zeros_like(mean), scale * torch.ones_like(mean))
...
class ReparamFullDense(nn.Module):
    ...
    def forward(self, x, num_samples=1):
        ...
        post_sample = sample_lr_gaussian(mean.view(1, -1), self.F, self.variance, num_samples, squeeze=True)
    ...
    def compute_kl(self):
        ...
        # scale = 1.
        scale = 2. / self.mean.weight.shape[0]
        return smart_gaussian_kl(mean, cov, torch.zeros_like(mean), torch.diag(scale * torch.ones_like(mean)))
